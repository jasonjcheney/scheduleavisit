...
def get_or_create_client(conn, provider_id: int, name: str, email: str, phone: str = "") -> int:
    ...
    cur = conn.execute(
        ...
        (provider_id, name.strip(), (email or "").strip(), (phone or "").strip(), now_iso()),
    )
    return int(cur.lastrowid)


def create_appointment(conn, provider_id, client_id, start, minutes, via="direct", referred_from=None):
    cur = conn.execute(
        """INSERT INTO appointments
           (provider_id, client_id, start_iso, duration_minutes, status, booked_via,
            referred_from_provider_id, created_at)
           VALUES (?,?,?,?, 'booked', ?, ?, ?)""",
        (provider_id, client_id, start.isoformat(timespec="seconds"), minutes, via, referred_from, now_iso()),
    )
    return int(cur.lastrowid)
...
async def api_login(request: Request):
    ...
    with db() as conn:
        ...
        resp = JSONResponse({"ok": True, "redirect": data.get("next") or "/dashboard", "name": u["name"]})
...
async def api_signup(request: Request):
    ...
    with db() as conn:
        ...
        notify(conn, uid, "welcome", "Welcome to ScheduleAVisit",
               "Share your booking link. Set a weekly hour cap. Invite a colleague you trust.")
...
def api_availability(slug: str, date: str):
    try:
    ...
    with db() as conn:
        ...
        data = availability_for(conn, u, day)
        data["ok"] = True
...
async def api_book(slug: str, request: Request):
    ...
    with db() as conn:
        ...
        appt_id = create_appointment(conn, u["id"], cid, start, minutes, "direct")
        notify(
            conn, u["id"], "booking",
            f"New visit — {name}",
            f"{name} booked {format_long(day)} at {format_time(hhmm)} ({minutes} min) on your public link.",
        )
        print(f"[book] {name} <{email}> with {u['slug']} on {day} {hhmm}", flush=True)
...
async def api_book_referral(slug: str, request: Request):
    ...
    with db() as conn:
        ...
        appt_id = create_appointment(conn, peer["id"], cid, start, minutes, "referral", origin["id"])
        notify(
