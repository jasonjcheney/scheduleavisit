...
4. App home / privacy: `https://scheduleavisit.onrender.com` and `/privacy`.
5. Scopes to add:
...
If `GOOGLE_REDIRECT_URI` is left as the sign-in callback (`…/auth/google/callback`), the app maps it to the calendar callback automatically. Register **both** URIs in Google either way.
